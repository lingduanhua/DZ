App.model.article = {
	...
	insertArticles : function(articles, successCallback){
		var remaining = articles.length, i, l, data = [];
        ...
        for (i = 0, l = articles.length; i < l; i = i + 1) {
            data[i] = [articles[i].title, articles[i].body, articles[i].length];
        }

        App.database.runQuery("INSERT INTO articles (title, lang, body, length) VALUES (?, ?, ?);", data, successCallback);
	},
